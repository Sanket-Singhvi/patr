...
pub async fn setup_routes(state: &AppState) -> Router {
	Router::new()
		...
		.mount_auth_json_endpoint(create_workspace, state)
		.mount_auth_json_endpoint(delete_workspace, state)
		.mount_auth_json_endpoint(get_workspace_info, state)
		.mount_auth_json_endpoint(is_name_available, state)
		.mount_auth_json_endpoint(update_workspace_info, state)
}
