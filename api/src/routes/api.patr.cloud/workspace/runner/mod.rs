...
pub async fn setup_routes(state: &AppState) -> Router {
	Router::new()
		...
		.mount_auth_json_endpoint(add_runner_to_workspace, state)
		.mount_auth_json_endpoint(remove_runner_from_workspace, state)
		.mount_auth_json_endpoint(list_runners_for_workspace, state)
		.mount_auth_json_endpoint(get_runner_info, state)
}
